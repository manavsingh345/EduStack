  ...
  const handleAutoGenerate = async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            // For Edit: Append or Replace? Let's Append if existing, or just set if empty.
            // Actually, usually AI generation implies starting fresh or adding to it. 
            // Let's just append for safety so we don't wipe existing work.
            setChapters([...chapters, ...generatedChapters]);
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-start md:p-8 p-4 pt-8'>
      ...
      {step === 0 && (
          <div className="w-full max-w-3xl bg-white p-6 rounded shadow-sm border mx-auto">
             <h2 className="text-2xl font-bold mb-6">Edit Course Basics</h2>
                     ...
                     <div className='flex flex-col gap-1 w-1/3'>
                        <label className="font-medium">Thumbnail</label>
                         <label htmlFor="thumbnailImage" className='flex items-center gap-2 cursor-pointer bg-blue-50 px-3 py-2 border border-blue-200 rounded text-blue-600 hover:bg-blue-100 transition'>
                             {image ? <span className='truncate'>{image.name}</span> : <span className='truncate'>Change Image</span>}
                         ...
                         {existingThumbnail && !image && (
                            <div className="mt-2">
                                <img src={existingThumbnail} alt="Current" className="w-16 h-10 object-cover rounded" />
                            </div>
                         )}
                    </div>
                </div>
      ...
      {step === 1 && (
          <div className="w-full max-w-3xl bg-white p-6 rounded shadow-sm border mx-auto">
             ...
             <div className="flex justify-between mt-6">
                ...
                <button onClick={() => setStep(2)} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">Next: Review & Update &rarr;</button>
             </div>
          </div>
      )}
      ...
      {step === 2 && (
          <div className="w-full max-w-5xl bg-white p-6 rounded shadow-sm border mx-auto text-center">
             <h2 className="text-3xl font-bold mb-4">Ready to Update?</h2>
             ...
             <CoursePreview courseData={{
                ...
                coursePrice,
                discount,
                image: image || (existingThumbnail ? existingThumbnail : null), // Handle URL string for preview if no new image
                courseContent: chapters
             }} />
